...
class Leveling(commands.Cog):
    def __init__(self, bot):
        ...
        self.xp_reward_range = tuple(self.bot.config.get('xp-reward-range', DEFAULT_XP_REWARD_RANGE))

        self.players = defaultdict(dict)
        self._get_level_xp = lambda n : 5*(n**2)+50*n+100

        self.font = Font(path=__file__[:-6] + "Aquire.otf", color=Color("white"), antialias=True)
    ...
    def get_player(self, member : discord.Object):
        ...
        player = Player(
            member, member.guild, self.bot.sync_redis)
    ...
    async def levelup_tracker(self, context : Context, player : Player, og_lvl : int, new_level : int):
    ...
    async def rank(self, ctx : Context, member : discord.Member = None):
        ...
        with Drawing() as draw:
            ...
            with Image(blob=self.template) as image:
                url = str(member.avatar.replace(static_format="png", size=256)).replace('gif', 'png')
                print(url)
                async with self.bot.session.get(url) as req:
                    with Image(blob=await req.read()) as img:
                        ...
                        with Image(blob=self.mask) as mask:
                            ...
                            img.transparent_color(Color("white"), alpha=0, fuzz=int(65535 * 0.03))
