...
link_crazy = re.compile(r"(?i)\b((?:https?:(?:/{1,3}|[a-z0-9%])|[a-z0-9.\-]+[.](?:com|net|org|edu|gov|mil|aero|asia|biz|cat|coop|info|int|jobs|mobi|museum|name|post|pro|tel|travel|xxx|ac|ad|ae|af|ag|ai|al|am|an|ao|aq|ar|as|at|au|aw|ax|az|ba|bb|bd|be|bf|bg|bh|bi|bj|bm|bn|bo|br|bs|bt|bv|bw|by|bz|ca|cc|cd|cf|cg|ch|ci|ck|cl|cm|cn|co|cr|cs|cu|cv|cx|cy|cz|dd|de|dj|dk|dm|do|dz|ec|ee|eg|eh|er|es|et|eu|fi|fj|fk|fm|fo|fr|ga|gb|gd|ge|gf|gg|gh|gi|gl|gm|gn|gp|gq|gr|gs|gt|gu|gw|gy|hk|hm|hn|hr|ht|hu|id|ie|il|im|in|io|iq|ir|is|it|je|jm|jo|jp|ke|kg|kh|ki|km|kn|kp|kr|kw|ky|kz|la|lb|lc|li|lk|lr|ls|lt|lu|lv|ly|ma|mc|md|me|mg|mh|mk|ml|mm|mn|mo|mp|mq|mr|ms|mt|mu|mv|mw|mx|my|mz|na|nc|ne|nf|ng|ni|nl|no|np|nr|nu|nz|om|pa|pe|pf|pg|ph|pk|pl|pm|pn|pr|ps|pt|pw|py|qa|re|ro|rs|ru|rw|sa|sb|sc|sd|se|sg|sh|si|sj|Ja|sk|sl|sm|sn|so|sr|ss|st|su|sv|sx|sy|sz|tc|td|tf|tg|th|tj|tk|tl|tm|tn|to|tp|tr|tt|tv|tw|tz|ua|ug|uk|us|uy|uz|va|vc|ve|vg|vi|vn|vu|wf|ws|ye|yt|yu|za|zm|zw|dev|xyz|app)/)(?:[^\s()<>{}\[\]]+|\([^\s()]*?\([^\s()]+\)[^\s()]*?\)|\([^\s]+?\))+(?:\([^\s()]*?\([^\s()]+\)[^\s()]*?\)|\([^\s]+?\)|[^\s`!()\[\]{};:'" + '"' + r".,<>?«»“”‘’])|(?:(?<!@)[a-z0-9]+(?:[.\-][a-z0-9]+)*[.](?:com|net|org|edu|gov|mil|aero|asia|biz|cat|coop|info|int|jobs|mobi|museum|name|post|pro|tel|travel|xxx|ac|ad|ae|af|ag|ai|al|am|an|ao|aq|ar|as|at|au|aw|ax|az|ba|bb|bd|be|bf|bg|bh|bi|bj|bm|bn|bo|br|bs|bt|bv|bw|by|bz|ca|cc|cd|cf|cg|ch|ci|ck|cl|cm|cn|co|cr|cs|cu|cv|cx|cy|cz|dd|de|dj|dk|dm|do|dz|ec|ee|eg|eh|er|es|et|eu|fi|fj|fk|fm|fo|fr|ga|gb|gd|ge|gf|gg|gh|gi|gl|gm|gn|gp|gq|gr|gs|gt|gu|gw|gy|hk|hm|hn|hr|ht|hu|id|ie|il|im|in|io|iq|ir|is|it|je|jm|jo|jp|ke|kg|kh|ki|km|kn|kp|kr|kw|ky|kz|la|lb|lc|li|lk|lr|ls|lt|lu|lv|ly|ma|mc|md|me|mg|mh|mk|ml|mm|mn|mo|mp|mq|mr|ms|mt|mu|mv|mw|mx|my|mz|na|nc|ne|nf|ng|ni|nl|no|np|nr|nu|nz|om|pa|pe|pf|pg|ph|pk|pl|pm|pn|pr|ps|pt|pw|py|qa|re|ro|rs|ru|rw|sa|sb|sc|sd|se|sg|sh|si|sj|Ja|sk|sl|sm|sn|so|sr|ss|st|su|sv|sx|sy|sz|tc|td|tf|tg|th|tj|tk|tl|tm|tn|to|tp|tr|tt|tv|tw|tz|ua|ug|uk|us|uy|uz|va|vc|ve|vg|vi|vn|vu|wf|ws|ye|yt|yu|za|zm|zw)\b/?(?!@)))")
emoji_crazy = re.compile(r"<(?P<animated>a?):(?P<name>[a-zA-Z0-9_]{2,32}):(?P<id>[0-9]{18,22})>")
invite_crazy = re.compile(r"(https?://)?(www.)?(discord.(gg|io|me|li)|discordapp.com/invite)/[^\s/]+?(?=\b)")
...
class AutoMod(commands.Cog):
    ...
    async def automod_cmd(self, ctx, check : str, choice : bool, action : str):
        checks = ['links', 'caps', 'mentions', 'emojis', 'spam', 'repeated_text', 'newlines', 'images', "zalgo", "all"]
    ...
    async def handle_checkfailure(self, error : AutoModCheckFailure, actions : dict):
        ...
        for action, values in actions.items():
            ...
            if action == "infraction":
                self.bot.dispatch('member_infraction', ctx.author, values.get('infractions', 1))
            if action == "kick":
                if ctx.me.guild_permissions.kick_members and ctx.me.top_role > ctx.author.top_role:
                    coros.append(ctx.author.kick(reason=f"Automod Check [{error.check}]:\n{error.message}"))
            if action == "ban":
                if ctx.me.guild_permissions.ban_members and ctx.me.top_role > ctx.author.top_role:
                    coros.append(ctx.author.ban(reason=f"Automod Check [{error.check}]:\n{error.message}"))
    ...
    async def validate_bypass(self, ctx : commands.Context, bypass : dict):
        for check, group in bypass.items():
            ...
            if check == "permissions":
                if getattr(ctx.channel.permissions_for(ctx.author), group.get('id'), False):
                    return True 
            if check == "member":
    ...
    async def check_newlines(self, context : commands.Context, data : dict):
        ...
        if newlines > amount:
            raise AutoModCheckFailure("newlines", context, f"Too many newlines ({newlines}/{per}s)")
    ...
    async def check_mentions(self, context : commands.Context, data : dict):
        ...
        if mentions > amount:
            raise AutoModCheckFailure("mentions", context, f"Too many mentions [{mentions}]")
    ...
    async def check_emojis(self, context : commands.Context, data : dict):
        ...
        emoji_count = sum(1 for _ in emoji_crazy.finditer(context.message.content)) + len([i for i in context.message.content if i in UNICODE_EMOJI['en']])
        if not emoji_count:
            return
    ...
    async def check_spam(self, context : commands.Context, data : dict):
        ...
        if messages > amount:
            raise AutoModCheckFailure("spam", context, f"Sending messages too quickly ({messages:,}/{per:,}s)")
    ...
    async def check_images(self, context : commands.Context, data : dict):
        ...
        if images > amount:
            raise AutoModCheckFailure("images", context, f"Sending images too quickly ({images:,}/{per:,}s)")
    ...
    async def checkmessage(self, ctx, message : discord.Message):
        ...
        yield discord.Embed(title='Checks Complete', description='```diff\n# Passed\n+' + '\n+'.join(passed) + ('\n-' if failed else '') + '\n-'.join([f"{name}: {e.message}" for (name, e) in failed]) + '\n```')     