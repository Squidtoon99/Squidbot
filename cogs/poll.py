...
class Confirm(discord.ui.View):
    ...
    async def handler(self, button, interaction):
        ...
        listdata = [
            (i, len([1 for k in data.values() if k == i])) for i in ["Yes", "No"]
        ]

        text = "\n".join(
            [
                f"{pkg[0]} | {pkg[1]} votes\n" + make_graph(pkg, listdata)
                for pkg in listdata
            ]
        )
    ...
    @discord.ui.button(
        label="👍", style=discord.ButtonStyle.green, custom_id="poll:confirm:yes"
    )
    async def _confirm(self, button, interaction):
        await self.handler(button, interaction)

    @discord.ui.button(
        label="👎", style=discord.ButtonStyle.red, custom_id="poll:confirm:no"
    )
    async def confirm(
...
def quickpoll(_bot, options):
    class QuickPoll(discord.ui.View):
        ...
        @discord.ui.select(
            options=[discord.SelectOption(label=i) for i in options],
            custom_id="quickpoll:selector:options",
        )
        async def selector(
            ...
            data = await self.bot.redis.hgetall(key)
            listdata = [
                (i, len([1 for k in data.values() if k == i]))
                for i in set(await self.bot.redis.hvals(key))
            ]
...
class Polls(commands.Cog):
    ...
    async def poll(
        self, ctx: commands.Context, question: str, *, flags: PollFlags = None
    ) -> dict:
        """
    ...
    async def quickpoll(self, ctx: commands.Context, *, question_and_choices: str):
        ...
        if len(choices) <= 2:
            raise commands.CommandError(
                "You must have at least `1` question and `2` choices"
            )
