...
class MySelect(discord.ui.Select):
    def __init__(
        ...
        options,
        *,
        custom_id="CommandRuleList:selector:1",
    ):
        super().__init__(
            custom_id=custom_id,
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.author_id:
            return await interaction.response.send_message(
                "This is not your selector", ephemeral=True
            )
        ...
        if data["type"] == "command":
            ...
            embed.description = "\n".join(
                [
                    f"{pos}. {cmd.qualified_name}"
                    for pos, cmd in enumerate(
                        sorted(cog.walk_commands(), key=lambda x: x.qualified_name),
                        start=1,
                    )
                ]
            )
...
class CommandRuleList(discord.ui.View):
    def __init__(self, ctx, selected=None):
        ...
        self.back = {
            "label": "back",
            "value": json.dumps({"type": "selector", "value": "back"}),
        }
        super().__init__(timeout=None)
        items = [
            dict(label=key, value=json.dumps({"type": "plugin", "value": key}))
            for key in ctx.bot.plugins.keys()
        ]
        if selected:
                        ...
                        value=json.dumps(
                            {
                                ...
                                "plugin": cmd.cog.qualified_name,
                            }
                        ),
                    )
                    for cmd in sorted(
                        ctx.bot.get_cog(v).walk_commands(),
...
class PermissionSelector(discord.ui.View):
    ...
    @discord.ui.select(
        custom_id="PermissionSelector:listscreen:1",
        options=[discord.SelectOption(label=i) for i in ["Whitelist", "Blacklist"]],
    )
    async def listselector(
        self, button: ui.Button, interaction: discord.Interaction
    ) -> None:
        await self.handler(b=button, i=interaction)

    @discord.ui.select(
        custom_id="PermissionSelector:choicescreen:1",
        options=[
...
class SortFlags(
    commands.FlagConverter, delimiter=" ", prefix="--", case_insensitive=True
):
    type: str = "all"
    group: str = "all"
...
class Permissions(commands.Cog):
    ...
    async def permissions_listrules(self, ctx):
        await ctx.reply(
            embed=discord.Embed(
                color=self.bot.color, description="Select a value to get started"
            ),
            view=CommandRuleList(ctx),
        )
